import numpy as np
from utils import norm, khatri_rao_power, khatri_rao_product, \
    ...
    for iter in range(maxiter):
        ...
        if k > 0:
            ...
            if ucorr[ind] > rho:
                Ak -= corr[ind] * A_[:, ind]
                Ak *= gamma / norm(Ak)
                Ak += (rho * np.sign(corr[ind])) * A_[:, ind] 
...
def spm_21sym_robust(T, r=None, **kwargs):
    ...
    A = np.zeros((m, r), order='F')
    B = np.zeros((n, r), order='F')
    l = np.zeros((r,))
    ...
    for k in range(r):
        ...
        alphaU = np.dot(Ak, U)
        alphaV = np.dot(Vt, (Ak.reshape(-1, 1) * Bk.reshape(1, -1)).reshape(-1))
