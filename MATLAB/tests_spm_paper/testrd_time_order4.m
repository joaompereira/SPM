clearvars
clc
addpath '../'
addpath '../helper_functions/'
addpath(genpath('../other_packages/'))

rng_seed = 0; % for reproducibility
rng(rng_seed);

disp('Running computation time experiment (order 4, Figure 2b)...');

% L and R values
Lval = round(exp(linspace(log(10),log(55),13)));
Rval = floor(Lval.^2/3);
nvals = length(Lval);
n = 4;

% Different Algorithms
% The tensor order has to be 4 for FOOBI to work
Algs = {'SPM' , @(T,L,R) subspace_power_method(T, L, 4);
        'FOOBI', @(T,L,R) foobi1_tensorlabplus(T, L, R);
        'NLS', @(T,L,R) tensorlab_ccpd_nls(T, L, 4, R);
        };

nalgs = size(Algs,1);

% Number of tries for each algorithm
% The variance of the running time is bigger
% for some algorithms. We run these more
% times to produce smoother results
ntries = 20;

% Time of execution and estimate error for different algorithms and 
% (L,R) values
time = cell(nalgs, 1);
error = cell(nalgs, 1);

for i=1:nalgs
    time{i} = NaN(nvals, ntries);
    error{i} = NaN(nvals, ntries);
end

%% 1st Call
% Because of MATLAB's inner workings, the
% 1st call to a function has overhead.
% Therefore, we call the algorithms for the 
% first time without recording timings.
L = Lval(1);
R = Rval(1);

% True rank decomposition
A_true = randn(L,R) / sqrt(L);

% Generate nD tensor
T = generate_lowrank_symtensor(A_true, n);

s = rng;

for i=1:nalgs

    % Use Algorithm i to get rank decomposition
    A_est = Algs{i,2}(T,L,R);
    
    rng(s);
end

%% The actual runs
for j=1:nvals
    
    L = Lval(j);
    R = Rval(j);
    fprintf('L =%3d ',L);
    k = 0;
   
    for tries=1:ntries

        while tries/ntries>k/20
            fprintf('\x2592')
            k = k+1;
        end

        % True rank decomposition
        A_true = randn(L,R) / sqrt(L);
    
        % Generate nD tensor
        T = generate_lowrank_symtensor(A_true, n);
        norm_T = norm(reshape(T, [], 1));

    
        s = rng;

        for i=1:nalgs
    
            if j>1 && ~(mean(time{i}(j-1,:),'omitnan') <= 100)
              continue
            end

            timer = tic;
            
            % Use Algorithm i to get rank decomposition
            A_est = Algs{i,2}(T,L,R);
            
            % Get time
            time{i}(j, tries) = toc(timer);

            % Calculate error
            error{i}(j, tries) = rderror(A_est, A_true, n) / norm_T;

            rng(s);
            
        end

    end

    fprintf('\n');
    
end

% Save results
clear T % T is cleared since it occupies a lot of space
save results/testrd_time_order4.mat

