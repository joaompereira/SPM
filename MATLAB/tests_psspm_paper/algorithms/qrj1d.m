function [A,B_factor,varargout]=qrj1d(T,varargin)
...
for i=2:n,
   ...
   scalars = vecnorm(inv(B));
   A = inv(B)./vecnorm(inv(B)); 
   B_factor = zeros(N,n);
   for i = 1:N
        Dmatrix = Y(:,1+(i-1)*n:i*n);
        B_factor(i,:) = diag(Dmatrix);
   end
   B_factor = B_factor.*scalars.^2;
